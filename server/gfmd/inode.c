	...
	gfarm_int32_t flags, struct gfarm_timespec *mtime)
{
	...
	if ((fo->flag & GFARM_FILE_CKSUM_INVALIDATED) != 0) {
		gflog_debug(GFARM_MSG_UNFIXED, "file checksum is invalidated");
	...
	struct inode **inp, int *createdp)
{
	...
	if ((e = inode_access(parent, user, GFS_W_OK)) != GFARM_ERR_NO_ERROR) {
		gflog_debug(GFARM_MSG_UNFIXED, "inode_access() failed: %s",
			gfarm_error_string(e));
		dir_remove_entry(parent->u.c.s.d.entries, name, len);
		return (e);
	}
...
gfarm_error_t
inode_replicated(struct file_replicating *fr, gfarm_int32_t errcode,
	gfarm_off_t size)
{
	struct inode *inode = fr->inode;
	int transaction = 0;
	...
	if (size != inode_get_size(inode) ||
		...
		inode_add_replica(inode, fr->dst, 1);
	}
...
remove_replica_internal(struct inode *inode, struct file_copy *copy)
{
	...
	if (e != GFARM_ERR_NO_ERROR)
		gflog_error(GFARM_MSG_1000328,
		    "host_remove_replica_enq(%lld, %s): %s",
		    (unsigned long long)inode->i_number, host_name(copy->host),
		    gfarm_error_string(e));
	...
	char ***hostsp, gfarm_int64_t **gensp, gfarm_int32_t **oflagsp)
{
	...
	if (latest_only)
		...
		ndead = host_count_dead_copies_all(inode_get_number(inode),
		    up_only);
	...
	if (e == GFARM_ERR_NO_ERROR && !latest_only)
		e = host_dead_copies_info_all(inode_get_number(inode), up_only,
		    &ndead, &hosts[i], &gens[i], &oflags[i]);
