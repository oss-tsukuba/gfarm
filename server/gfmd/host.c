...
gfm_server_host_info_set(struct peer *peer, int from_client, int skip)
{
	...
	if (!from_client || user == NULL || !user_is_super_admin(user)) {
		...
		gflog_debug(GFARM_MSG_1005154, "%s (%s@%s) during read_only",
...
gfm_server_host_info_modify(struct peer *peer, int from_client, int skip)
{
	...
	if (!from_client || user == NULL || !user_is_super_admin(user)) {
		...
		gflog_debug(GFARM_MSG_1005155, "%s (%s@%s) during read_only",
		    diag, peer_get_username(peer), peer_get_hostname(peer));
		e = GFARM_ERR_READ_ONLY_FILE_SYSTEM;
		needs_free = 1;
...
gfm_server_host_info_remove(struct peer *peer, int from_client, int skip)
{
	...
	if (!from_client || user == NULL || !user_is_super_admin(user)) {
		...
		gflog_debug(GFARM_MSG_1005156, "%s (%s@%s) during read_only",
		    diag, peer_get_username(peer), peer_get_hostname(peer));
		e = GFARM_ERR_READ_ONLY_FILE_SYSTEM;
	} else
