...
gfarm_error_t
gfm_server_setxattr_by_inode(
	struct peer *peer, int from_client, int skip, int xmlMode)
{
	const char *diag = xmlMode ?
	...
	e = gfm_server_get_request(peer, diag,
	    "llsBi", &inum, &gen, &attrname, &size, &value, &flags);
	if (e != GFARM_ERR_NO_ERROR) {
	...
	if (xmlMode) {
		...
		if (((char *)value)[size - 1] != '\0') {
			e = GFARM_ERR_INVALID_ARGUMENT;
			gflog_debug(GFARM_MSG_UNFIXED, "%s: %lld:%lld: %s",
	...
	if ((process = peer_get_process(peer)) == NULL) {
		...
		gflog_debug(GFARM_MSG_UNFIXED, "%s: %lld:%lld: %s", diag,
		    (long long)inum, (long long)gen, gfarm_error_string(e));
	} else if ((e = xattr_access(xmlMode, inode, user,
			    attrname, GFS_W_OK, XATTR_OP_SET))
		   != GFARM_ERR_NO_ERROR) {
		gflog_debug(GFARM_MSG_UNFIXED, "%s: %lld:%lld: %s", diag,
		    (long long)inum, (long long)gen, gfarm_error_string(e));
	...
	struct peer *peer, int from_client, int skip, int xmlMode)
{
	...
	e = gfm_server_get_request(peer, diag, "lls", &inum, &gen, &attrname);
	if (e != GFARM_ERR_NO_ERROR) {
		gflog_debug(GFARM_MSG_UNFIXED,
		    "%s: %s", diag, gfarm_error_string(e));
		return (e);
	...
	if ((process = peer_get_process(peer)) == NULL) {
		...
		gflog_debug(GFARM_MSG_UNFIXED, "%s: %lld:%lld (%s): %s", diag,
		    (long long)inum, (long long)gen, attrname,
		    gfarm_error_string(e));
	} else if (value == NULL) { /* not cached */
		db_waitctx_init(&waitctx);
		waitctx_initialized = 1;
...
gfarm_error_t
gfm_server_removexattr_by_inode(struct peer *peer, int from_client, int skip,
	int xmlMode)
{
	const char *diag = xmlMode ?
