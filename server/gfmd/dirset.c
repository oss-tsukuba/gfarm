...
dirset_new(const char *dirsetname, struct user *u)
{
	...
	quota_metadata_init(&ds->q);
	quota_usage_clear(&ds->q.usage); /* dirset is empty at first */
	ds->user = u;
	ds->dir_count = 0;
	...
	struct gfarm_dirset_info *di, struct quota_metadata *q)
{
	...
	ds->q = *q;
#if 0 /* DQTODO - until dirquota_check is implemented */
	quota_usage_init(&ds->q.usage); /* usage in backend DB is garbage */
#endif
...
static void
dirset_all_for_dirsets(struct dirsets *sets,
...
gfm_server_dirset_info_set(struct peer *peer, int from_client, int skip)
{
	...
	if (!from_client) {
		...
		e = db_quota_dirset_add(username, dirsetname, &ds->q);
		if (e != GFARM_ERR_NO_ERROR)
			gflog_error(GFARM_MSG_UNFIXED,
...
dirset_reply(void *closure, struct user *u)
{
	...
	sets = user_get_dirsets(u);
	if (sets != NULL)
		dirset_all_for_dirsets(sets, client, dirset_reply_one);
}
...
gfm_server_dirset_info_list(struct peer *peer, int from_client, int skip)
{
	...
	if (!from_client) {
		...
		count = 0;
		if (all_users) {
			user_all(&count, dirset_count_add, 1);
		} else {
		...
		if (count == 0) {
			...
			if (all_users) {
				user_all(peer_get_conn(peer), dirset_reply, 1);
			} else {
...
gfm_server_quota_dirset_get(struct peer *peer, int from_client, int skip)
{
	...
	if (!from_client) {
		...
		e = GFARM_ERR_NO_SUCH_OBJECT;
	} else if (!quota_is_checked(&ds->q.usage)) {
...
quota_dirset_put_reply(struct peer *peer, struct dirset *ds, const char *diag)
{
	...
	if (!quota_is_checked(&ds->q.usage)) {
		/* quota_check hasn't been done yet */
		e = GFARM_ERR_FILE_BUSY;
	} else {
		e = GFARM_ERR_NO_ERROR;
		quota_exceed_to_grace(ds->q.limit.grace_period, &ds->q.exceed,
		    &grace);
	}
	return (gfm_server_put_reply(peer, diag, e, "sslllllllllllllllll",
	    user_name(ds->user),
	    ds->dirsetname,
	    ds->q.limit.grace_period,
	    ds->q.usage.space,
	    grace.space_time,
	    ds->q.limit.soft.space,
	    ds->q.limit.hard.space,
	    ds->q.usage.num,
	    grace.num_time,
	    ds->q.limit.soft.num,
	    ds->q.limit.hard.num,
	    ds->q.usage.phy_space,
	    grace.phy_space_time,
	    ds->q.limit.soft.phy_space,
	    ds->q.limit.hard.phy_space,
	    ds->q.usage.phy_num,
	    grace.phy_num_time,
	    ds->q.limit.soft.phy_num,
	    ds->q.limit.hard.phy_num));
}
...
dir_reply_dirset(void *closure, struct dirset *ds)
{
	...
	c->ds = ds;
	quota_dir_all_for_dirset(ds->dir_list, c, dir_reply_quota_dir);
}
...
dir_count_add(void *closure, struct user *u)
{
	...
	if (all_dirsets) {
		sets = user_get_dirsets(u);
		if (sets != NULL)
			dirset_all_for_dirsets(sets, c, dir_count_add_dirset);
	} else {
...
dir_reply(void *closure, struct user *u)
{
	...
	if (all_dirsets) {
		sets = user_get_dirsets(u);
		if (sets != NULL)
			dirset_all_for_dirsets(sets, c, dir_reply_dirset);
	} else {
		ds = user_lookup_dirset(u, c->dirsetname);
		if (ds != NULL)
...
gfm_server_quota_dir_list(struct peer *peer, int from_client, int skip)
{
	...
	if (!from_client) {
		...
		if (all_users) {
			user_all(&closure, dir_count_add, 1);
		} else {
