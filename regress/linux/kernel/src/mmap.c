...
int mmaptest(char *filename, int mmapflags, char *writedata, char *updatedata,
	...
	void *addr = mmap(NULL, mmaplen, PROT_READ | PROT_WRITE, mmapflags,
		fd, 0);
	if (addr == MAP_FAILED) {
		ret = 1;
		perror("mmap() failed\n");
...
int main(int argc, char *argv[])
{
	...
	int ret = mmaptest(filename, MAP_SHARED, "12345678", "abcd", 2,
			"12abcd78");
	ret += mmaptest(filename, MAP_PRIVATE, "12345678", "abcd", 2,
			"12345678");
