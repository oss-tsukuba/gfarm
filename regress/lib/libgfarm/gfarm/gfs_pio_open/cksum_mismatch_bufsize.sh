...
( ( $gfs_pio_test -r -R $bufsize -A $bufsize,0,-1 $GFARM_TEST_CKSUM_MISMATCH >/dev/null ) 2>&1 | egrep "$PAT" >/dev/null ) || exit $exit_fail
...
( ( $gfs_pio_test -r -A 0,0,$bufsize -S $bufsize -I $GFARM_TEST_CKSUM_MISMATCH > /dev/null ) 2>&1 | egrep "$PAT" >/dev/null ) || exit $exit_fail
