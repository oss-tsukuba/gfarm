...
dbq_init(struct dbq *q)
{
	...
	cond_init(&q->finished, msg, "finished");
...
dbq_wait_to_finish(struct dbq *q)
{
	...
	while (!q->quited) {
		...
		cond_wait(&q->finished, &q->mutex, msg, "finished");
	}
...
dbq_waitret(struct db_waitctx *ctx)
{
	...
	mutex_lock(&ctx->lock, msg, "lock");
	while (ctx->e == UNINITIALIZED_GFARM_ERROR) {
		cond_wait(&ctx->cond, &ctx->lock, msg, "cond");
	}
	e = ctx->e;
	mutex_unlock(&ctx->lock, msg, "lock");
	return (e);
}
...
dbq_delete(struct dbq *q, struct dbq_entry *entp)
{
	...
	if (q->n <= 0) {
		assert(q->quitting);
		q->quited = 1;
		cond_signal(&q->finished, msg, "finished");
...
db_host_add(const struct gfarm_host_info *hi)
{
	...
	if (h == NULL) {
		gflog_debug(GFARM_MSG_UNFIXED, "db_host_dup() failed");
		return (GFARM_ERR_NO_MEMORY);
	}
...
db_user_add(const struct gfarm_user_info *ui)
{
	...
	if (u == NULL) {
		gflog_debug(GFARM_MSG_UNFIXED, "db_user_dup() failed");
		return (GFARM_ERR_NO_MEMORY);
	}
...
db_group_add(const struct gfarm_group_info *gi)
{
	...
	if (g == NULL) {
		gflog_debug(GFARM_MSG_UNFIXED, "db_group_dup() failed");
		return (GFARM_ERR_NO_MEMORY);
	}
...
db_inode_add(const struct gfs_stat *st)
{
	...
	if (i == NULL) {
		gflog_debug(GFARM_MSG_UNFIXED, "db_inode_dup() failed");
		return (GFARM_ERR_NO_MEMORY);
	}
