...
# readdir 1 (empty)
{
	...
	n=`/bin/ls -la ${MOUNTPOINT} | grep "^drwxrwxr-x \+2 \+${ADMUSER} \+${ADMGROUP} .\+ \.$" | wc -l`
	if [ "${n}" -ne 1 ]; then
...
# readdir 2 (1 dir)
{
	...
	n=`/bin/ls -la --full-time ${MOUNTPOINT} | grep "^drwxrwxr-x \+3 \+${ADMUSER} \+${ADMGROUP} \+. ${timestampA} \.$" | wc -l`
	if [ "${n}" -ne 1 ]; then
		echo "WARN: timestamp maybe incorrect"
	fi
}
...
# readdir 3 (2 dir)
{
	...
	n=`/bin/ls -la --full-time ${MOUNTPOINT} | grep "^drwxrwxr-x \+4 \+${ADMUSER} \+${ADMGROUP} \+. ${timestampBB} \.$" | wc -l`
	if [ "${n}" -ne 1 ]; then
		echo "WARN: timestamp maybe incorrect"
	fi
}
...
# readdir 4 (1 dir, after rmdir)
{
	...
	n=`/bin/ls -la --full-time ${MOUNTPOINT} | grep "^drwxrwxr-x \+3 \+${ADMUSER} \+${ADMGROUP} \+. ${timestampBB} \.$" | wc -l`
	if [ "${n}" -ne 1 ]; then
		echo "WARN: timestamp maybe incorrect"
	fi
